...
read, compact():
    - Stable-sorts the buffered updates.
    - Appends them to the main vector.
    - Merges the old levels and new updates.
    - Groups entries with the same price.
    - Keeps the latest value for each price.
    - Removes levels whose latest quantity is zero.
...
- The project uses two input types.
    ...
    - Updates
        ...
        - b: Bid updates.
        - a: Ask updates.
- A quantity greater than zero adds a new level or replaces the quantity at an existing price. A quantity of zero removes the level at that price.
...
- PriceLevel
    ...
    """
    - Prices and quantities use fixed-point integer values instead of floating-point numbers. The scale is 10^9, which supports nine decimal places.
- PriceLevelContainer
...
- BookContainer
    ...
    - The map provides average constant-time symbol lookup. The vector provides compact storage and good iteration locality.
- Engine Cache
    - While processing updates, Engine caches symbol-to-book pointers. This avoids repeating the full BookContainer lookup for every event of a symbol. The cache is cleared after pending updates are compacted.

Algorithms
- Fixed-Point Conversion
    - Decimal strings are converted to unsigned 64-bit integers using a multiplier of 10^9. This avoids floating-point comparison and rounding problems in the order book.
- Snapshot Loading
    - For each book side:
        - Parse all price and quantity pairs.
        - Move them into the main vector.
        - Stable-sort them in the required order.
        - Group duplicate prices.
        - Keep the last value for each price.
        - Remove zero-quantity levels.
    - Snapshot sorting has O(nlogn) time complexity.
- Update Processing
    - Each update is appended to the overflow buffer in amortized O(1) time.
    - When compaction is required:
        - The buffer is stable-sorted in O(klogk).
        - The sorted buffer is merged with the main data.
        - Equal prices are resolved in one linear pass.
    - Here, 'n' is the number of active levels and 'k' is the number of buffered updates. The merge and duplicate-removal work is linear in the combined data size.
    - Stable ordering is important because the latest update for the same price must win.
- Deletion
    - A zero-quantity update is kept until compaction. If it is the latest update for its price, the corresponding level is not written back to the active vector.
- Best Price
    - Because each side remains sorted, the best price is available from the first element in O(1) time.
...
- Updates in the files are already in the order in which they should be applied.
- The latest snapshot or update entry for the same price is authoritative.
- Live exchange connectivity and snapshot synchronization are outside the project scope.
- The current implementation is single-threaded.
- Input files are expected to follow the supported format. The readers are optimized for this format rather than being general JSON parsers.
...
.\test\run_tests.bat
"""
- The suite covers the main data structures, fixed-point utilities, snapshot and update processing, boundary cases, invalid order values, missing files, missing symbols, and support for 2,000 books.
...
"""
- More information about benchmark measures is located into benchmark directory of the current project.